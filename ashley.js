 ...
 $("#btn-four").on("click", function(){
   ...
   var queryURL = "https://api.themoviedb.org/3/movie/" + movieId + "?api_key=a60b3a8bc5eefd424c235053a3248b79&append_to_response=videos,images"
    $.ajax({
        ...
        var movieTitle = $("<h1>").text("Movie Title: " + response.title).addClass("text-center")
        var tagline = $("<p>").text("" + response.tagline).addClass("text-center").attr("font-style:", "italic")
        var moviePlot = $("<p>").text("Summary: " + response.overview)
        var runtime = $("<p>").text("Runtime: " + response.runtime)
        var releaseDate = $("<p>").text("Release Date: " + response.release_date)
        $("#generated-content").append(movieTitle, movieTrailer, tagline);
        var movieTrailer = $("<div>").
        $("#generated-content").append(moviePlot, runtime, releaseDate);
